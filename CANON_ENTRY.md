# Canon Entry — Odin Agent Shell v7.1

Odin Agent Shell v7.1 is a **Windows-first, local-first, protocol-bound Universal Semantic Work Kernel**, **Small-Model Performance OS**, and optional **Internal Semantic IRC precompute engine**.
...
→ Odin validation and precompute
→ smallest sufficient worker
→ Candidate Artifact
→ app-owned rendering and apply decision
```

## Canon laws

1. No LLM in app.
2. Every AI-like task is Universal Work.
3. Odin outputs candidates, not reality changes.
4. Apps own state, apply, sends, storage, business logic and domain authority.
5. Models are projection workers only.
6. Semantic Bus coordinates; it does not authorize or mutate.
7. Local-first, remote only by explicit permission.
8. Small-model-first, 3B + 7B/8B hybrid default.
9. Larger models are escalation tiers, not default architecture.
10. Claim/evidence/reality gates are mandatory.

## Current repository role

This repo is a **Canon-Lock + Full Docs Expansion + Codex Build Prep**. It is not yet a completed runtime. Any build work must preserve the canon and raise capability through tests, schemas, registries, and implementation receipts.
