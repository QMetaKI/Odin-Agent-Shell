# Storage, Trace and Retention v7.1 — Deep Subsystem Spec
...
Storage supports traceability, cache, app pairing, model profiling and candidate reproducibility. Storage is not an app database and does not own app state.
...
| --- | --- | --- |
| App | caller_manifests bindings app_pairings | caller identity and policy |
| Work | universal_works input_artifacts output_contracts | work records |
| Precompute | context_capsules system_profiles seed_profiles fit_scores | pre-model artifacts |
| Bus | semantic_bus_events channels topics batches replays | internal bus |
| Model | model_profiles model_runs model_responses model_dojo_results | provider/model history |
| Candidate | candidate_artifacts candidate_dna response_packets | outputs |
| Quality | minichecks critic_reports conflicts | review |
| Trace | trace_entries receipt_candidates | audit/debug |


## Retention Policy

Default retention must be conservative:

```text
bus events: short retention unless trace export requested
raw payloads: avoid persistence unless allowed
candidate artifacts: persist only according to user/app settings
model outputs: cache by digest and privacy policy
support bundles: explicit export only
```
