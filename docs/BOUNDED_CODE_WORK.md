# Bounded Code Work v7.1 — Deep Subsystem Spec
...
Bounded Code Work ensures code-related tasks remain candidate-only unless an app-owned apply tool later acts. Odin may create PatchPlan Candidate, Review Candidate, Debug Hypothesis Candidate and Command Preview Candidate. It may not claim execution outcomes without receipt.


## Code Flow

repo_context -> code lens -> Thor lite/full when needed -> worklet graph -> slot forge -> model route -> critic cascade -> patchplan_candidate or review_candidate -> response packet.


## Required Boundaries

No direct patch write, no test-result assertion, no host-state assertion, no deployment assertion, no hidden shell execution. Command previews remain candidates.
