# Local Mediation Protocol v7.1 — Deep Subsystem Spec
...
Local Mediation Protocol is the exchange contract between app, Odin, semantic bus and model providers. It ensures every message has direction, boundary and typed payload.


## Artifact Directions

App -> Odin: app message, event digest, context response, Universal Work. Odin -> App: context request, response packet, conflict. Odin internal: binding, system profile, semantic event, worklet graph, slot contract, minicheck, candidate DNA, trace, receipt candidate. Odin -> Model: ModelWorkPacket. Model -> Odin: ModelResponse.


## Conflict Handling

A conflict is returned when Odin cannot safely proceed but can explain why. Conflict response may request context, state digest, permission adjustment or app-side review. Conflict is not a runtime failure; it is a safe boundary outcome.
