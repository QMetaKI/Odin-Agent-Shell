# Security and Privacy v7.1 — Deep Subsystem Spec
...
Odin is local-first and app-sovereign. Remote access is blocked unless explicitly allowed by caller policy and user settings. Internal Semantic Bus is local-only. Secrets are redacted from traces and bus payloads.
...
| --- | --- | --- |
| App State | digest/context refs | Odin mutation |
| Network | localhost | WAN/LAN by default |
| Remote Model | explicit opt-in | implicit fallback |
| Semantic Bus | local coordination | public IRC |
| Models | bounded slot projection | authority decisions |
| Traces | redacted debug | secret storage |


## Secret Rules

Secrets must not be placed in channel names, topics, trace titles, cache keys, model prompts or exported support bundles. Payload scanner should mark possible secrets and either redact or block according to privacy class.
