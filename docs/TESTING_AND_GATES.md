# Testing and Gates v7.1 — Deep Subsystem Spec
...
Tests are compiler-like gates. The repo must fail early when docs, schemas, registries, examples, validators or claim boundaries drift.


## Gate Families

| Gate | Purpose | Minimum Test |
| --- | --- | --- |
| Universal Work | all work is bounded | valid/invalid work examples |
| No LLM in App | apps remain bridge-only | template scan |
| Candidate Output | no direct app action | candidate_only checks |
| Semantic Bus | local-only coordination | event validation/replay |
| Model Ladder | sweet spot default | route class tests |
| Claim Boundary | no unsupported assertions | claim scanner |
| Registry Parity | docs and registries align | registry load tests |
| Schema Strictness | contracts are parseable | json schema validation |
| Docs Coverage | canonical specs complete | anchor+length tests |
...
direct_apply_request
remote_without_permission
blocked_sensitive_artifact
semantic_bus_external_channel
semantic_event_missing_trace
model_output_forbidden_claim
cache_bypasses_gate
app_qirc_full_mirror_attempt
```
