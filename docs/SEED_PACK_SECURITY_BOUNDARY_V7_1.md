# Seed Pack Security Boundary v7.1
...
Seed Pack Security Boundary v7.1 defines a bounded, typed, app-provided Seed Pack system for Odin. Apps may ship seed packs that Odin can ingest, validate, compile, score, activate, and bind into QIRC, Odin Core, Y*, Fairy DSL, Shadow Runtime, Runtime Packs, Model Routing and Output Composition without granting the app or seed pack execution authority.

## Required anchors

- Seed Pack Security Boundary
...
Threats include seed prompt injection, remote permission smuggling, app-state mirror attempts, hidden executable payloads, overbroad semantic influence, conflicting safety seeds, model escalation pressure, and false capability claims. Odin mitigates these with manifest allowlists, schema validation, trust tiering, privacy class checks, conflict resolution, seed fanout caps, and Why Trace disclosure.
