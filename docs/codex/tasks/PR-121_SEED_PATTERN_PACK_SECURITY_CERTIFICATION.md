...
Implement the Seed Pattern Pack Security Certification scope as a candidate-only public repo and Windows build readiness preparation slice. This task is part of v0.7.5 and must preserve all existing v7.1 boundaries.
...
```text
Scope: PR-121 — Seed Pattern Pack Security Certification
Changed files:
Validation:
