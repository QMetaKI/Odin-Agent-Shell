...
Implement the Windows Installer Update Rollback Drilldown scope as a candidate-only public repo and Windows build readiness preparation slice. This task is part of v0.7.5 and must preserve all existing v7.1 boundaries.
...
```text
Scope: PR-119 — Windows Installer Update Rollback Drilldown
Changed files:
Validation:
