# Thor Integration v7.1 — Deep Subsystem Spec
...
Thor Bridge imports candidate-only handoff/review discipline where useful. Odin does not need Thor for every small request, but uses it for repo/code/patchplan, multi-agent handoff, review and receipt candidate workflows.


## Modes

none: no Thor. lite: return contract only. full: handoff candidate with guard and expected output. review: candidate review. receipt: receipt candidate. Thor never grants apply authority inside Odin.
